...
def fuzzy(img, level=128, p=2):
    ...
    L. K. Huang and M. J. J. Wang, “Image thresholding by minimizing the measures of fuzziness,” Pattern Recognit., vol. 28, no. 1, pp. 41–51, 1995.
    ...
    nvox = img.size
    count = np.asarray([((img >= t-step/2) & (img < t+step/2)).sum() for t in np.arange(low, high, step)])
    weighted = np.arange(low, high, step) * count
    backcount = np.asarray([(img < t+step/2).sum() for t in np.arange(low, high, step)])
